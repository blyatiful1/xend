...
8. Planning on the main model and building with cheap, verified subagents keeps the main context tiny
   ...
   briefs in output tokens. The plain layers did save 4-5% on the same long tasks.
...
- Conclusion across r6, r7 and r7b: plain xend was cheaper than solo Sonnet on all three long tasks (-4% to -5%, fewer turns each time), the first favourable cost result after five micro-task runs; architect mode was never cheaper (+99% to +251% against solo Sonnet, +39% to +67% for Fable against solo Fable) at equal quality, because builders' cold prefixes and the planner's brief output outweighed the reading it kept out of the main context. Architect mode is therefore opt-in in every profile.
