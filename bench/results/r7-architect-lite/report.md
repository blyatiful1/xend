...
Base arm: `solo-sonnet`, compared against 6 other arm(s): archlite-fable, archlite-sonnet, plain-sonnet, solo-fable, arch-sonnet, arch-fable.

## archlite-fable vs solo-sonnet

Paired comparison of `archlite-fable` against `solo-sonnet` on 2 tasks (14 runs).

| Metric | solo-sonnet | archlite-fable | change |
|---|---|---|---|
...
| Tokens per task (all models incl. subagents, all four meters) | 1247073 | 1648618 | 39.4% (95% CI -45.4% to 124.3%) |
| Uncached input tokens per task | 36023 | 64666 | 79.5% |
| Output tokens per task | | | 151.2% |
| Turns per task | 19.5 | 29.5 | 10.0 |
| Cost per task (USD, Claude Code reported, subagents included) | 0.510 | 3.322 | 553.0% (95% CI 303.0% to 803.0%) |
| Cost split (main model / subagents) | $0.510 / $0.000 | $3.282 / $0.040 | main 543.0%, sub n/a |
...
- cost: FAIL (one-sided 95% upper bound of cost change 803.0%; must be below 0%)
- turns: INCONCLUSIVE (one-sided 95% upper bound of turn delta 22.00; must be at most +0.25)

| Category | tasks | pass delta | token change |
|---|---|---|---|
| project | 2 | +0.0 pp | 39.4% |

| Task | cat | pass base | pass treat | score base | score treat | tokens base | tokens treat | ratio | turns Δ |
|---|---|---|---|---|---|---|---|---|---|
| project-ledger-cli | project | 100% | 100% | 100% | 100% | 1140964 | 2558764 | 2.24 | 22.0 |
| project-log-pipeline | project | 100% | 100% | 100% | 100% | 1353182 | 738471 | 0.55 | -2.0 |

| Arm | model | tokens per task | cost per task |
|---|---|---|---|
| archlite-fable | claude-fable-5-1 | 1540254 | $3.282 |
| archlite-fable | claude-haiku-4-5-20251001 | 108364 | $0.040 |
| solo-sonnet | claude-sonnet-5 | 1247073 | $0.510 |

## archlite-sonnet vs solo-sonnet

Paired comparison of `archlite-sonnet` against `solo-sonnet` on 2 tasks (14 runs).

| Metric | solo-sonnet | archlite-sonnet | change |
|---|---|---|---|
| Pass rate | 100.0% | 100.0% | +0.0 pp (95% CI +0.0 pp to +0.0 pp) |
| Score (fraction of hidden tests passed) | 100.0% | 100.0% | +0.0 pp (95% CI +0.0 pp to +0.0 pp) |
| Tokens per task (all models incl. subagents, all four meters) | 1247073 | 5000533 | 305.6% (95% CI 251.1% to 360.2%) |
| Uncached input tokens per task | 36023 | 10695 | -70.3% |
| Output tokens per task | | | -66.0% |
| Turns per task | 19.5 | 9.0 | -10.5 |
| Cost per task (USD, Claude Code reported, subagents included) | 0.510 | 1.792 | 251.1% (95% CI 248.5% to 253.6%) |
| Cost split (main model / subagents) | $0.510 / $0.000 | $1.299 / $0.493 | main 154.5%, sub n/a |
...
**Verdict: FAIL** (all three must pass)
- quality: PASS (one-sided 95% lower bound of pass-rate delta +0.0 pp vs gate -3 pp)
- cost: FAIL (one-sided 95% upper bound of cost change 253.6%; must be below 0%)
- turns: PASS (one-sided 95% upper bound of turn delta -8.00; must be at most +0.25)

| Category | tasks | pass delta | token change |
|---|---|---|---|
| project | 2 | +0.0 pp | 305.6% |

| Task | cat | pass base | pass treat | score base | score treat | tokens base | tokens treat | ratio | turns Δ |
|---|---|---|---|---|---|---|---|---|---|
| project-ledger-cli | project | 100% | 100% | 100% | 100% | 1140964 | 5250242 | 4.60 | -8.0 |
| project-log-pipeline | project | 100% | 100% | 100% | 100% | 1353182 | 4750823 | 3.51 | -13.0 |

| Arm | model | tokens per task | cost per task |
|---|---|---|---|
| archlite-sonnet | claude-haiku-4-5-20251001 | 1510722 | $0.493 |
| archlite-sonnet | claude-sonnet-5 | 3489811 | $1.299 |
| solo-sonnet | claude-sonnet-5 | 1247073 | $0.510 |

## plain-sonnet vs solo-sonnet

Paired comparison of `plain-sonnet` against `solo-sonnet` on 2 tasks (14 runs).
...
Paired comparison of `solo-fable` against `solo-sonnet` on 2 tasks (14 runs).
...
Paired comparison of `arch-sonnet` against `solo-sonnet` on 2 tasks (14 runs).
...
Paired comparison of `arch-fable` against `solo-sonnet` on 2 tasks (14 runs).
