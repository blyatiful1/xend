...
# whatever the agent (or reference/apply.sh) put under ./invsys.
set -u
...
if [ -n "$summary_line" ]; then
  # count passes even when other tests failed or errored ("2 failed, 1 error, 54 passed")
